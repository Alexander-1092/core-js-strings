...
function removeFirstOccurrences(str, value) {
  const indexItem = str.indexOf(value);
  const startStr = str.slice(0, indexItem);
  const nextStr = str.slice(indexItem + value.length);
  return indexItem === -1 ? str : startStr + nextStr;
}
...
function removeLastOccurrences(str, value) {
  const indexItem = str.lastIndexOf(value);
  const startStr = str.slice(0, indexItem);
  const nextStr = str.slice(indexItem + value.length);
  return indexItem === -1 ? str : startStr + nextStr;
}
...
function sumOfCodes(str) {
  if (typeof str !== 'undefined' && str !== null) {
    if (str.length > 0) {
      const arrNums = str.split('').map((item) => item.charCodeAt());
      return arrNums.reduce((acc, item) => acc + item);
    }
    return 0;
  }
  return 0;
...
function isPalindrome(str) {
  const newStr = str.toLocaleLowerCase().replaceAll(/['!?, ]/g, '');
  const strRevers = newStr.split('').reverse().join('');
  const strOrig = str.toLocaleLowerCase().replaceAll(/['!?, ]/g, '');
  return strRevers === strOrig;
}
...
function findLongestWord(sentence) {
  const newArr = sentence.split(' ').sort((a, b) => {
